...
import { WriteBlockData, ArduinoCodeblockData } from "../../code_generator/types"
...
type WriteBlockProps = {
  setBlock: (l: WriteBlockData & { label: string }) => void
  leave: WriteBlockData & { label: string }
  removeBlock: (l: WriteBlockData & { label: string }) => void
  moveRight: (l: WriteBlockData & { label: string }) => void
  moveLeft: (l: WriteBlockData & { label: string }) => void
}

export const WriteBlock = (props: WriteBlockProps) => {
  ...
  return (
    <Card style={{ minHeight: 150, borderRadius: 20, marginBottom: 50 }}>
      ...
      <p>Write the value of <PopoverValue
        popoverElem={setValue}
        tagElem={props.leave.value}
      /> to port <PopoverValue
          popoverElem={setPort}
